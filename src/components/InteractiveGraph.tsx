...
const InteractiveGraph: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[650px] flex items-center justify-center pointer-events-none">
      ...
      <div 
        ref={centralNodeRef}
        className="relative z-20 w-44 h-44 rounded-full border-2 border-saffron bg-obsidian/90 backdrop-blur-md flex items-center justify-center text-center p-4 pointer-events-auto cursor-pointer group hover:border-saffron transition-colors duration-700 shadow-[0_0_60px_rgba(184,134,11,0.15)]"
      >
        <div className="absolute inset-2 rounded-full border border-saffron/40 group-hover:border-saffron transition-colors duration-700" />
        <div className="absolute inset-0 rounded-full bg-saffron/5" />
        
        <div className="flex flex-col items-center">
          <span className="font-mono text-[9px] tracking-[0.3em] uppercase text-saffron mb-2">Your</span>
      ...
      {nodes.map((node, i) => {
        ...
        return (
          <div
            ...
            className="absolute z-10 w-20 h-20 rounded-full glass-card border border-black/50 flex flex-col items-center justify-center text-center p-2 pointer-events-auto cursor-pointer group hover:border-saffron transition-all duration-500 hover:shadow-[0_0_30px_rgba(242,204,143,0.15)]"
            onMouseEnter={() => handleNodeHover(node.id)}
          >
            <div className="text-black/40 group-hover:text-saffron transition-colors duration-500 transform group-hover:scale-110">
              {node.icon}
            </div>
            <span className="font-mono text-[9px] mt-2 tracking-tighter uppercase text-black/30 group-hover:text-parchment transition-colors duration-500 whitespace-nowrap">
              {node.label}
            </span>
            
            {/* Subtle orbital ring */}
            <div className="absolute inset-[-4px] rounded-full border border-black/50 group-hover:border-saffron transition-colors duration-500" />

            {/* Hover Tooltip */}
            <div className="absolute top-full mt-4 left-1/2 -translate-x-1/2 w-48 p-3 rounded-lg bg-[#050401]/95 border border-saffron/80 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-50">
